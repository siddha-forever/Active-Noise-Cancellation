...
for j = 1:length(mus)
    for i = 1:100
        ...
        file_path = '/Users/siddha-book/Downloads/Gender-Recognition/audio.wav'; % Replace with the actual path to your signal file
        [x, fs] = audioread(file_path);
        x = x / max(abs(x)); % Normalize the input signal
        N = length(x); % Length of input signal
...
% Plot output signal (filtered signal)
subplot(2, 1, 2)
t = (0:length(d)-1) / fs; % Time axis in seconds
plot(t, d)
...
output_file_path = '/Users/siddha-book/Downloads/active-noise-cancellation-master/Code/output.wav'; % Replace with the desired output file path

% Normalize the filtered signal
d_normalized = d / max(abs(d));

% Write the filtered signal to an audio file
audiowrite(output_file_path, d_normalized, fs);

disp('Output audio file saved successfully.');
